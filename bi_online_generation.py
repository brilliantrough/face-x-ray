...
class BIOnlineGeneration():
    ...
    def gen_one_datapoint(self):
        ...
        # randomly choose a real face or a fake face
        if data_type == 'fake':
        ...
        if random.randint(0, 1):
            ...
            # randomly use bilinear or nearest interpolation
            if random.randint(0, 1):
                face_img = face_img.resize(
                    (aug_size, aug_size), Image.Resampling.BILINEAR)
            else:
                face_img = face_img.resize(
                    (aug_size, aug_size), Image.Resampling.NEAREST)
            face_img = face_img.resize((600, 600), Image.Resampling.BILINEAR)
    ...
    def get_blended_face(self, background_face_path):
        ...
        foreground_face = sktransform.resize(
            foreground_face, background_face.shape, preserve_range=True).astype(np.uint8)
        ...
        blended_face = sktransform.resize(
            blended_face, (600, 600), preserve_range=True).astype(np.uint8)
    ...
    def search_similar_face(self, this_landmark, background_face_path):
        ...
        all_candidate_path = random.sample(
            self.data_list, k=self.search_sample_k)
...
def generate_blends(obj, path_suffix='', valid=False):
    ...
    for i in tqdm(range(200)):
       # try:
        ...
        filename = 'blend_dataset' + extension + '/' + \
            label + '_masks/' + str(base_name) + '.jpg'
     ...
     #   print(e)
    all_paths = {'image paths': img_paths,
                 'mask paths': mask_paths, 'image labels': img_labels}
    if valid:
        save_name = 'blend_dataset' + extension + \
            '/'+'all_paths_valid_'+path_suffix+'.pkl'
    else:
